...
<liferay-ui:header backLabel="Back" backURL="<%= backURL %>" title="Indietro" />
...
<c:if test="<%= testataDocumento != null %>">
	...
	<liferay-ui:error key="errorSavingArticoliAssociatoOP" message="Errore nel salvataggio dei dati" />
	<liferay-ui:error key="errorUpdating" message="Errore nel salvataggio dei dati." />
	<c:if test="<%= message != null %>">
		<span class="portlet-msg-info"><%= message %></span>
	</c:if>
	<aui:form action="<%= validateDocument %>" method="post" name="fm">
...
<%-- <c:if test='<%= !r.getCodiceArticolo().equals("") %>'> --%>
					...
					<liferay-ui:search-container-column-text name="Prezzo" value='<%= r.getPrezzo() != 0 ? String.valueOf(r.getPrezzo()) : "" %>'/>
					<c:choose>
						<c:when test="<%= !r.getVerificato() %>">
							<liferay-ui:search-container-column-jsp align="right" name="Verifica" path="/jsps/action-check-row.jsp" />
						</c:when>
						<c:otherwise>
							<liferay-ui:search-container-column-text name="Verifica" value="" />
						</c:otherwise>
					</c:choose>
<%-- </c:if> --%>
